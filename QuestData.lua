...
----------------------------------------------------------------------
-- Get active area world quests (world quests the player has walked
-- into). Uses multiple detection strategies:
-- 1. Scan quest log including hidden entries for isTask world quests
-- 2. Use C_TaskQuest.GetQuestsForPlayerByMapID for map-based detection
-- 3. Check C_QuestLog.IsOnQuest or GetLogIndexForQuestID to confirm
--    the player has activated the quest by entering the area
----------------------------------------------------------------------
function TTQ:GetActiveWorldQuests()
    local quests = {}
    if not C_QuestLog then return quests end

    local found = {} -- questID → true, to deduplicate

    -- Strategy 1: Scan the full quest log including hidden entries.
    -- World quests appear as isTask=true, sometimes with isHidden=true.
    local numEntries = C_QuestLog.GetNumQuestLogEntries()
    for i = 1, numEntries do
        local info = C_QuestLog.GetInfo(i)
        if info and not info.isHeader and info.questID then
            ...
            if info.isTask then
                ...
                if isWorldQuest and not found[questID] then
                    found[questID] = true
                    self:_addActiveWorldQuest(quests, questID, info)
                end
            end
        end
    end
...
----------------------------------------------------------------------
-- Helper: build a world quest entry and add it to the list
----------------------------------------------------------------------
function TTQ:_addActiveWorldQuest(quests, questID, info)
    local objectives = C_QuestLog.GetQuestObjectives(questID)
    local isComplete = C_QuestLog.IsComplete(questID) and true or false
    local pct, fulfilled, required = self:CalcProgress(objectives)

    -- Get title: try C_TaskQuest first, then quest log info
    ...
    if (not title or title == "") and C_QuestLog.GetLogIndexForQuestID then
        local logIdx = C_QuestLog.GetLogIndexForQuestID(questID)
        if logIdx then
            local logInfo = C_QuestLog.GetInfo(logIdx)
            if logInfo then title = logInfo.title end
        end
    end
    title = title or "World Quest"
    ...
    local questType = ClassifyQuest(questID, classifyInfo)

    -- Time remaining
    local timeLeft = nil
    if C_TaskQuest and C_TaskQuest.GetQuestTimeLeftMinutes then
        timeLeft = C_TaskQuest.GetQuestTimeLeftMinutes(questID)
    end
    ...
    table.insert(quests, {
        ...
        difficultyLevel    = info.difficultyLevel or 0,
        questType          = questType,
        frequency          = info.frequency or 0,
        isComplete         = isComplete,
        isSuperTracked     = isSuperTracked,
        isTask             = true,
        isBounty           = info.isBounty or false,
        objectives         = objectives or {},
        progress           = pct,
        fulfilled          = fulfilled,
        required           = required,
